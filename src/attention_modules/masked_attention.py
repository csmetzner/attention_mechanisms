"""
    This file contains source code for masked attention proposed by Feng et al. 2022
    (https://ieeexplore.ieee.org/document/9676694).
...
class MaxMaskedAttention(nn.Module):
    """
    Max masked target attention with trainable query matrices.
    ...
    def forward(self, H: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        if self._multihead:
            ...
            mask_max = torch.randn(A.size()).bool().to(device)
            for b in range(A.size()[0]):
                max_val = torch.max(A[b])
                mask_max[b][A[b] < self._gamma * max_val] = False
            A = A.masked_fill(mask_max, 0)
...
class RankedMaskedAttention(nn.Module):
    """
    Ranked masked target attention with trainable query matrices.
